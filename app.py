...
def handle_config():
    current_settings = {
        "duration": 10, "mode": "slideshow", "newest_count": 5, 
        "brightness": 100, "night_mode": False, 
        "night_start": "22:00", "night_end": "07:00", 
        "night_brightness": 20, "night_duration": 30,
        "forced_image": None # NEU: Wenn gesetzt, wird nur dieses Bild gezeigt
    }
    ...
    for filename in files:
        ...
        if ext in ['.jpg', '.jpeg', '.png', '.webp', '.mp4']:
            ...
            media_list.append({
                ...
                "focus_x": crop_data['x'], "focus_y": crop_data['y']
            })
...
def delete_image():
    ...
    # Clean settings if forced image is deleted
    settings = load_json(SETTINGS_FILE, {})
    if settings.get('forced_image') == filename:
        settings['forced_image'] = None
    ...
    for f in os.listdir(IMAGE_FOLDER):
        if f.startswith(base_name) and f != filename:
            try: os.remove(os.path.join(IMAGE_FOLDER, f))
            except: pass
...
def upload_image():
    ...
    if 'file' in request.files:
        ...
        if file.filename:
            if token and token != UPLOAD_SECRET: return jsonify({"error": "Wrong Token"}), 403
